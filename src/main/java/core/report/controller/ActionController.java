package core.report.controller;
...
import core.config.ConfigAllure;
        ...
        try{
            if (CONFIG_ALLURE.allureEnable()){
                final String URL = String.format("%s:%s/allure-docker-service/send-results?project_id=%s&force_project_creation=%s",
                        ...
                        URLEncoder.encode(CONFIG_ALLURE.allureProject()+"-"+DriverManager.getBrowserName(),StandardCharsets.UTF_8.name()),
